...
* **`FILE_ID`**: エクセルでいう「ファイル指定（ID）」です。（開発者側で設定済み）
* **`SHEET_MONDAI`**: エクセルでいう「問題データが入っているシートタブ名」です。
* **`SHEET_CATEGORY`**: エクセルでいう「カテゴリ設定が入っているシートタブ名」です。
...
* **「合言葉（Passcode）を変更・削除したい」**
  * 学校マスタ内の `admnin` シートにある `Passcode` 列の文字を直接書き換えるだけで、即座にWeb側にも反映されます。
* **「出題するカテゴリや問題数を調整したい」**
  * 学校マスタ内のカテゴリ設定シート（例: `Cat_kokoro`）に記載されている数値（`count`）を変更するだけで調整できます。
* **「問題が読み込まれない」**
